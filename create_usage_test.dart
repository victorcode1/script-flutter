  ...
  group('usageValues', () {
    ...
    setUp(() {
      testbed = Testbed(setup: () {
        ...
        final List<String> templatePaths = <String>[
          ...
          globals.fs.path.join('flutter', 'packages', 'flutter_tools', 'templates', 'victordev'),
    ...
    testUsingContext('set template type as usage value', () => testbed.run(() async {
      ...
      expect((await command.usageValues).commandCreateProjectType, 'skeleton');
         ...
         await runner.run(<String>['create', '--no-pub', '--template=victordev', 'testy7']);
      expect((await command.usageValues).commandCreateProjectType, 'victordev');
